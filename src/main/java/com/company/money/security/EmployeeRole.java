...
@ResourceRole(name = "Employee", code = EmployeeRole.CODE, scope = SecurityScope.UI)
public interface EmployeeRole extends UiMinimalRole {
    ...
    @EntityAttributePolicy(entityClass = PaymentRequest.class, attributes = {
            "requestNumber", "requestDate", "status", "applicant", "counterparty",
            "nomenclature", "counterpartyBankAccount", "contract", "amount", "purpose",
            "requestedPaymentDate", "attachments", "lastChangedBy", "missingInOneC"
    }, action = EntityAttributePolicyAction.MODIFY)
    @EntityAttributePolicy(entityClass = PaymentRequest.class, attributes = {
            "approvedPaymentDate", "cfoComment", "oneCDocumentRef", "oneCDocumentNumber",
            "oneCDocumentDate", "oneCDocumentTitle", "oneCExportPayload", "deferredUntil"
    }, action = EntityAttributePolicyAction.VIEW)
    @EntityPolicy(entityClass = PaymentRequest.class, actions = {
            EntityPolicyAction.READ, EntityPolicyAction.CREATE, EntityPolicyAction.UPDATE, EntityPolicyAction.DELETE
    })
    void paymentRequest();

    @EntityAttributePolicy(entityClass = PaymentRequestFile.class, attributes = "*",
            action = EntityAttributePolicyAction.MODIFY)
    @EntityPolicy(entityClass = PaymentRequestFile.class, actions = {EntityPolicyAction.ALL})
    ...
    @EntityAttributePolicy(entityClass = PaymentRequestNotice.class, attributes = "*",
            action = EntityAttributePolicyAction.VIEW)
    @EntityPolicy(entityClass = PaymentRequestNotice.class, actions = {EntityPolicyAction.READ})
